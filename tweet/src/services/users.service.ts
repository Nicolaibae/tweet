...
class UserService {
  ...
  private SignRefrehToken({ user_id, verify, exp }: { user_id: string, verify: UserVerifyStatus, exp?: number }) {
    if (exp) {
      return signToken({
        payload: {
          user_id,
  ...
  private deocodeRefreshToken(refresh_token: string) {
    return verifyToken(refresh_token, process.env.JWT_SECRET_REFRESH_TOKEN as string)
  }
  ...
  async register(payload: registerReqBody) {
    ...
    const {iat,exp}= await this.deocodeRefreshToken(refresh_token)
  ...
  async refreshToken({
    ...
    const [new_access_token, new_refresh_token] = await Promise.all([
     ...
     const decoded_refresh_token= await this.deocodeRefreshToken(refresh_token)
    await databaseService.refreshTokens.insertOne(
      new RefreshToken({
        user_id: new ObjectId(user_id),
